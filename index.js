...
// APS endpoints - These will work on Render, fail gracefully on Vercel
app.post('/api/aps/calculate-time', async (req, res) => {
    try {
        ...
        const pythonProcess = spawn('python3', [
            '-c',
            `
from albayrak_aps_scheduler import AlbayrakAPSScheduler
import json
...
print(json.dumps({'time': time}))
        ...
        pythonProcess.on('close', (code) => {
            if (code !== 0) {
                ...
                return res.status(500).json({ error: 'APS not available on this server' });
            }
        ...
        // Will fail on Vercel but work on Render
        console.log('APS endpoint called - only works on Render');
        res.status(503).json({ error: 'APS service only available on Render deployment' });
    }
});
